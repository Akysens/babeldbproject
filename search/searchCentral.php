...
<head>
    ...
    <title>BABEL - Search Media</title>
...
<body>
    <div class="navbar-wrapper">
        <ul class="navbar">
            ...
            <li class="navbarelem"><a class="navbarelemlink" href="../input/inputCentral.php">Input</a></li>
            <li class="navbarelem"><a class="navbarelemlink" href="searchCentral.php">Search</a></li>
        </ul>
    </div>

    <div class="card-wrapper" id="search-bar">
        <form action="">
            <label for="input-type-selector" class="input-label">Search:</label>
            <select id="input-type-selector" >
                <option selected disabled hidden value="none">Please choose an option.</option>
                <option value="albumslongerthan/form.php">Albums Longer Than a Given Time</option>
                <option value="digitalworksby/form.php">Digital Works Done By Someone</option>
                <option value="gameslargersize/form.php">Games Larger Than a File Size</option>
            </select>
            <input type="button" value="Continue" class="input-button" onclick="goToSelected()">
        </form>
    </div>

    <div class="card-wrapper" id="search-all">
        <div>
            <form action="searchCentral.php" method="post" autocomplete="off">
                <div class="form-input-block">
                    <label class="input-label" for="search-name">Search media by name:</label>
                    <input id="search-name" name="name" class="input-text-box autocomplete" type="text" placeholder="Title to search.">
                    <input type="submit" value="Search">
                    <div class="acscript"></div>
                    <script>
                        $(document).ready(function() {
                            $("#search-name").keyup("keyup", function() {
                                $.ajax({
                                    url: "query.php",
                                    data: {text: $("#search-name").val()},
                                    success: function (result) {
                                        $(".acscript").html(result);
                                    },
                                    error: function () {
                                        alert("acscript");
                                }})
        ...
        <div class="seach-by-name-results">
            <?php
                include "req.php";
                ...
                if(isset($_REQUEST['name'])) {
                    ...
                    $sql = "SELECT Media.mid, Media.title, Media.genre, Media.datePublished FROM Media
                            WHERE title LIKE '%" . $r_name . "%'
                            ORDER BY title;";
                    ...
                    if ($result) {
                        if($result->num_rows > 0) {
                            ...
                            echo "<th class=\"long-table-data\">Title</th>";
                            echo "<th class=\"short-table-data\">Genre</th>";
                            echo "<th class=\"short-table-data\">Published</th>";
                            echo "</tr>";
                            ...
                            echo "<p>No results found for title: " . $r_name . "</p>";
                        }
                    }
                    else {
                        echo "<p>Search <b>failed</b>: " . $connection->error . "</p>";
                    }
