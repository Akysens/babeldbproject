...
<body>
    ...
    <div class="card-wrapper search-result-card">
        ...
        <?php
            ...
            // Convert duration from seconds to required format.
            $h = floor($r_duration / 3600);
            $m = ($r_duration / 60) % 60;
            $s = $r_duration % 60;
            $converted_duration = sprintf("%02d:%02d:%02d", $h, $m, $s); 

            $sql = "SELECT Media.mid, Media.title, Media.genre, Media.datePublished, DiscsAndTapes.duration FROM Media
                    INNER JOIN PhysicalMedia on PhysicalMedia.mid = Media.mid
                    INNER JOIN DiscsAndTapes on DiscsAndTapes.mid = PhysicalMedia.mid
                    WHERE TIME(duration) > TIME('" . $converted_duration . "') AND genre LIKE '%Album%'
            ...
            if ($result) {
                if($result->num_rows > 0) {
                    ...
                    echo "<th class=\"short-table-data\">Title</th>";
                    echo "<th class=\"short-table-data\">Genre</th>";
                    echo "<th class=\"short-table-data\">Published</th>";
                    echo "<th class=\"short-table-data\">Duration</th>";
                    echo "</tr>";
                    while($row = $result->fetch_assoc()) {
                        echo "<tr>";
                        echo "<td><a class=\"hidden-link\" href=\"details.php?mid=" . $row["mid"] . "\">" . $row["title"] . "</a></td>";
                    ...
                    echo "<p>No results found for duration: " . $converted_duration . "</p>";
                }
            }
            else {
            ...
            echo "<a href=\"../searchCentral.html\">Go back to maintenance page.</a>";
